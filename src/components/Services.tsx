...
export function Services({ activeService, setActiveService }: ServicesProps) {
  ...
  return (
    <div className="relative bg-gradient-to-b from-white via-emerald-100 to-white py-16 sm:py-24" id="services">
      {/* Decorative elements */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">Choose Your Service</h2>
          ...
          <div className="inline-flex p-1 space-x-1 bg-white/90 backdrop-blur-sm rounded-xl shadow-sm">
            {Object.entries(categories).map(([key, category]) => (
              <button
                ...
                className={`px-4 sm:px-6 py-2 text-sm font-medium rounded-lg whitespace-nowrap transition-all duration-200 ${
                  activeCategory === key
            ...
            {categories[activeCategory as keyof typeof categories].services.map((service, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="relative bg-white/90 backdrop-blur-sm p-6 sm:p-8 rounded-2xl shadow-sm hover:shadow-lg transition-all duration-200 h-full flex flex-col">
                    <div className="flex items-center mb-6">
                      <div className="flex items-center justify-center h-12 w-12 sm:h-14 sm:w-14 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 text-white group-hover:scale-110 transition-transform duration-200">
                    ...
                    <p className="text-gray-500 mb-6 flex-1 text-sm sm:text-base">{service.description}</p>
                    
                    <div className="space-y-3">
                      {service.features.map((feature, featureIndex) => (
                        <div key={featureIndex} className="flex items-start text-gray-600 text-sm sm:text-base">
                          <CheckCircle2 className="w-5 h-5 text-emerald-500 mr-2 flex-shrink-0 mt-0.5" />
                          <span>{feature}</span>
                        </div>
                      ))}
                      <button className="mt-6 w-full bg-gradient-to-r from-emerald-600 to-teal-500 text-white px-6 py-3 rounded-xl font-medium hover:opacity-90 transition-opacity duration-200 shadow-sm hover:shadow-md text-sm sm:text-base">
                        Book Now
                      </button>
                    </div>
                  </div>
